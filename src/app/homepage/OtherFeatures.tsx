  ...
  return (
    <div className='w-full p-10 space-y-16'>
        <div className='flex justify-center mx-auto text-3xl quote w-1/2 relative'>
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" className='absolute -top-4 left-0'>
        ...
        <section className='grid items-center justify-center space-y-2 xl:space-y-0 grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 w-full'>
            {otherData.map((data) => (
                <div key={data.id} className='bg-slate-900 group rounded-lg h-72 mx-2 p-4'>
                    <div className='relative p-2 flex flex-col  rounded-lg border-slate-950 ease-in duration-200 group-hover:border-slate-600 justify-center space-y-6 items-center border-2 h-full'>
                        <div className='mt-2 text-slate-950 group-hover:text-slate-600'>
                            <data.icon />
                        </div>
                        ...
                        <span className='absolute tracking-normal -top-[26px] -left-[1.5px] py-1 px-4 rounded-tl-lg border-r-8  border-b-8 border-slate-900 bg-slate-950 font-medium text-slate-800 group-hover:text-slate-600 ease-in duration-200'>0{data.id + 1}</span>
                    </div>                    
                </div>
            ))}
        </section>
